...
class GameWorld():
    ...
    def __init__(self, seed, worlddimensions, screendimensions):
        ...
        pygame.init()
        screen = pygame.display.set_mode(screendimensions,pygame.RESIZABLE)
...
#            self.sprites.add(n)
    ...
    def doMouseUp(self):
        ...
        self.agent.turnToFace([offsetX, offsetY])
        self.agent.shoot()
        
    def doMouseMove(self):
        pos = pygame.mouse.get_pos()
        offsetX = pos[0] + self.agent.position[0] - self.camera[0]
        offsetY = pos[1] + self.agent.position[1] - self.camera[1]
        self.agent.turnToFace([offsetX, offsetY])

    def doMouseMoveTo(self):
        pos = pygame.mouse.get_pos()
        offsetX = pos[0] + self.agent.position[0] - self.camera[0]
        offsetY = pos[1] + self.agent.position[1] - self.camera[1]
    ...
    def doKeyDown(self, key):
        ...
        if not (insideObstacle(p1, self.obstacles) or insideObstacle(p2, self.obstacles) or insideObstacle(p3, self.obstacles) or insideObstacle(p4, self.obstacles)):
            c=1.5*self.agent.getRadius()
            self.agent.move((x,y))

            
            if keys[K_LSHIFT] and not (x==0.0 and y==0.0):
                angle=math.degrees(numpy.arctan2(x,y))-90.0
                c=1.5*self.agent.getMaxRadius()
                vector = (c*math.cos(math.radians(angle)), -c*math.sin(math.radians(angle)))
                p1=(self.agent.rect.topleft[0]+vector[0],self.agent.rect.topleft[1]+vector[1])
                p2=(self.agent.rect.topright[0]+vector[0],self.agent.rect.topleft[1]+vector[1])
                p3=(self.agent.rect.bottomleft[0]+vector[0],self.agent.rect.topleft[1]+vector[1])
                p4=(self.agent.rect.bottomright[0]+vector[0],self.agent.rect.topleft[1]+vector[1])
                if not (insideObstacle(p1, self.obstacles) or insideObstacle(p2, self.obstacles) or insideObstacle(p3, self.obstacles) or insideObstacle(p4, self.obstacles)):

...
class Hero(MOBAAgent):
    ...
    def update(self, delta = 0):
        ...
        enemies = self.world.getEnemyNPCs(self.getTeam())+self.world.getEnemyTowers(self.getTeam())+self.world.getEnemyBases(self.getTeam())
        if enemies:
            dist=float('Inf')
            target=None
            for e in enemies:
                d=distance(self.getLocation(),e.getLocation())
                if d<dist:
                    dist=d
                    target=e.getLocation()
            angle = math.degrees(numpy.arctan2(-self.getLocation()[0]+target[0],-self.getLocation()[1]+target[1]))-90
            healer_vector = rotateVector(self.node_vectors[0], angle)
            companion1_vector = rotateVector(self.node_vectors[1], angle)

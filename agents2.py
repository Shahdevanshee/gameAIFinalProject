...
class KillMinion(BTNode):
    ...
    def execute(self, delta = 0):
        ...
        if self.target == None or distance(self.agent.getLocation(), self.target.getLocation()) > BIGBULLETRANGE:
            ...
            if (distance(self.agent.getLocation(),self.dest)<=2.0*self.agent.getMaxRadius() or rayTraceWorld(self.agent.getLocation(),self.dest,self.agent.world.getLines())!=None)  and self.destinations :
                self.dest=getRandomDestination(self.destinations)
            if self.dest:
                drawCross(self.agent.world.background, self.dest, (200, 0, 0), 4,2)
                self.agent.navigateTo(self.dest)
            if self.agent.canAreaEffect() and distance(self.agent.getLocation(),self.target.getLocation())<=AREAEFFECTRANGE:
...
class DodgeDaemon(BTNode):
    ...
    def execute(self, delta = 0):
        ...
        if self.agent.canDodge():
            ...
            for b in bullets:
                if distance(self.agent.getLocation(),b.getLocation())<self.agent.getMaxRadius() and b.getOwner()!=self.agent:
                    destinations=getDestinationsOnRadius(self.agent,3*self.agent.getMaxRadius(),self.agent.getLocation())                 
                    if destinations:   
